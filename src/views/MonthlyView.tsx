...
const formatDate = (date: Date) =>
  `${WEEKDAY[date.getUTCDay()]} ${date.getUTCDate()} ${new Intl.DateTimeFormat("fr-FR", { month: "short", timeZone: "UTC" }).format(date).replace(".", "")}.`;
const formatRange = (item: SchoolBreak) => {
  const fmt = (date: Date) =>
    `${date.getUTCDate()} ${new Intl.DateTimeFormat("fr-FR", { month: "short", timeZone: "UTC" }).format(date).replace(".", "")}.`;
...
type Props = {
  ...
  onAdjust: (field: keyof Entry, delta: number) => void;
};
...
export function MonthlyView({
  ...
  const showZones = (["A", "B", "C"] as Zone[]).some((schoolZone) =>
    (SCHOOL_BREAKS[String(startYear)]?.[schoolZone] ?? []).some(
      (item) =>
        (item.name === "Hiver" || item.name === "Printemps") && overlaps(item),
    ),
  );
    ...
    <div className="monthly-view">
      ...
      <section className="input-row work-row">
        ...
        <div className="stepper percent-stepper">
          <button
            onClick={() => onAdjust("workRate", -5)}
            aria-label="Réduire le temps de travail"
          >
            −
          </button>
          <output>{entry.workRate} %</output>
          <button
            className={entry.workRate >= 100 ? "disabled" : ""}
            onClick={() => onAdjust("workRate", 5)}
            aria-label="Augmenter le temps de travail"
          >
            +
          </button>
        </div>
      ...
      <section className="absence-section">
        ...
        <InputRow
          ...
          label="Congés payés"
          value={entry.leave}
          onMinus={() => onAdjust("leave", -0.5)}
          onPlus={() => onAdjust("leave", 0.5)}
        />
        <InputRow
          icon={Coffee}
          iconClass="rtt-icon"
          label="RTT"
          value={entry.rtt}
          onMinus={() => onAdjust("rtt", -0.5)}
          onPlus={() => onAdjust("rtt", 0.5)}
        />
        <InputRow
          icon={GraduationCap}
          ...
          onMinus={() => onAdjust("training", -0.5)}
          onPlus={() => onAdjust("training", 0.5)}
        />
        <InputRow
          ...
          label="Autres"
          value={entry.other}
          onMinus={() => onAdjust("other", -0.5)}
          onPlus={() => onAdjust("other", 0.5)}
        />
      </section>
      <section className="calendar-card">
        ...
        <div className="calendar-events">
          ...
          {schoolBreaks.map((item) => (
            <div className="calendar-event" key={`${item.name}-${item.start}`}>
