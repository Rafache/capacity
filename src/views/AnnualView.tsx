...
export function AnnualView({ entries, stats, annualBaseline, annualAvailable, annualUnavailable, annualRate, annualWorkRate, annualStats, csvOpen, onCsvToggle, onImport, onExport, onClear, onMonthOpen }: Props) {
  return (
    <div className="annual-view">
      ...
      <section className="monthly-bars">
        <div className="table-heading"><h2>Capacité par mois</h2><CsvMenu open={csvOpen} onToggle={onCsvToggle} onImport={onImport} onExport={onExport} onClear={onClear} /></div>
        <div className="annual-table" role="table" aria-label="Capacité mensuelle en jours">
          <div className="annual-table-header" role="row">
            ...
            {SEGMENTS.map(({ key, label, icon: Icon }) => <span className={key} key={key} role="columnheader" aria-label={label} title={label}><Icon aria-hidden="true" /></span>)}
          </div>
          {stats.map((item, index) => (
            <button className="annual-table-row annual-row-button" role="row" key={MONTHS_SHORT[index]} onClick={() => onMonthOpen(index)} aria-label={`Ouvrir ${MONTHS_SHORT[index]}`}>
              <span className="month-cell" role="rowheader"><strong>{MONTHS_SHORT[index]}{entries[index].note ? " •" : ""}</strong><small>{item.baseline} ouvrés</small></span>
